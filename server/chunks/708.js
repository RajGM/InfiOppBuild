...
/* harmony export */   "findMemberId": () => (/* binding */ findMemberId),
...
const allChannels = {
    Welcome: {
        "id": "1096841097231208569",
        general: "1096841097231208571",
        "get-roles": "1121770463278923787",
        announcement: "1121763370798940181"
    },
    Hackathon: {
        "id": "1121756472787877888",
        onsite: "1121756556527157289",
        remote: "1121756587548221531",
        hybrid: "1121756623078166548"
    },
    Grants: {
        "id": "1121756653541404702",
        travel: "1121756677973221406",
        course: "1121756703692685392",
        conference: "1121756725826031646"
    },
    Conferences: {
        "id": "1121756923323232336",
        design: "1121757199472005201",
        "launch-event": "1121757238588084296"
    },
    Internship: {
        "id": "1121757310012903464",
        "onsite": "1121757336600592385",
        "remote": "1121757363179896872",
        "hybrid": "1121757383862009917"
    },
    Meetup: {
        "id": "1121767943248818248",
        "meet-up": "1121768105618706557",
        chat: "1121768161738493973",
        "voice-chat": "1121768196026933339"
    }
};
async function findMemberId(username) {
    let id = undefined;
    await fetch(`${baseurl}/v9/guilds/${serverID}/members/search?query=${username}&limit=1`, {
        ...
        data.forEach((element)=>{
            if (element.user.username.toLowerCase() == username.toLowerCase()) id = element.user.id;
            return;
        });
    }).catch((error)=>{
        // Handle any errors
...
async function sendMessage(obj, discordUserID, category) {
    const embed_message = {
        "embeds": [
            {
                "title": obj.eventN,
...
async function editMessage(obj, discordUserID, category, messageID) {
    console.log("obj:", obj);
    const embed_message = {
        "embeds": [
            {
